...
function loadDrugsData() {
  ...
  if (drugsDataCache && cacheTimestamp && (Date.now() - cacheTimestamp < CACHE_DURATION)) {
    return drugsDataCache;
  }
...
router.get('/search', (req, res) => {
  try {
    ...
    for (let [drugName, conditions, sideEffects] of drugsData) {
      ...
      // Exact drug name match gets highest score
      if (drugLower === query) {
        matchScore = 1.0;
        matchType = 'exact_drug_match';
      }
      // Partial drug name match
